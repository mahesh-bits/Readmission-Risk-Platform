...
(function (global, factory) {
    var undef = "undefined";
    var nsKey, key, nm, theExports = {}, modName = "es5_ai_3_3_11", msMod="__ms$mod__";
    var mods={}, modDetail=mods[modName]={}, ver="3.3.11";
...
function objExtend(obj1, obj2, obj3, obj4, obj5, obj6) {
    ...
    var argLen = theArgs[_DYN_LENGTH$4 ];
    var deep = false;
    var idx = 1;
...
function _getNamedValue(values, name) {
    if (values) {
        for (var i = 0; i < values[_DYN_LENGTH$4 ]; i++) {
            var value = values[i];
            if (value[_DYN_NAME$2 ]) {
                if (value[_DYN_NAME$2 ] === name) {
                    return value;
                }
            }
        }
...
function findNamedServerTiming(name) {
    ...
    if (perf) {
        ...
        value = _getNamedValue((navPerf[_DYN_LENGTH$4 ] > 0 ? navPerf[0] : {}).serverTiming, name).description;
    }
    return value;
}
...
function redactQueryParameters(url, config) {
    ...
    for (var i = 0; i < sensitiveParams[_DYN_LENGTH$4 ]; i++) {
    ...
    if (queryString && queryString[_DYN_LENGTH$4 ]) {
        var pairs = queryString[_DYN_SPLIT$1 ]("&");
        for (var i = 0; i < pairs[_DYN_LENGTH$4 ]; i++) {
            var pair = pairs[i];
            if (!pair) {
                continue;
            }
            ...
            if (equalsIndex === -1) {
                ...
                if (paramValue === STR_EMPTY$1) {
                    ...
                    for (var j = 0; j < sensitiveParams[_DYN_LENGTH$4 ]; j++) {
...
function createCookieMgr(rootConfig, logger) {
    ...
    function _removePendingCookie(name) {
        if (_pendingCookies) {
            for (var i = _pendingCookies[_DYN_LENGTH$4 ] - 1; i >= 0; i--) {
...
function parseTraceParent(value, selectIdx) {
    ...
    if (!value || !isString(value) || value[_DYN_LENGTH$4 ] > 8192) {
        return null;
    }
    if (value[_DYN_INDEX_OF ](",") !== -1) {
        var values = value[_DYN_SPLIT$1 ](",");
        value = values[selectIdx > 0 && values[_DYN_LENGTH$4 ] > selectIdx ? selectIdx : 0];
    }
    ...
    try {
        if (response && response !== "") {
            ...
            if (result && result[_DYN_ITEMS_RECEIVED$1 ] && result[_DYN_ITEMS_RECEIVED$1 ] >= result.itemsAccepted &&
                result.itemsReceived - result.itemsAccepted === result.errors[_DYN_LENGTH$4 ]) {
                return result;
            }
        }
    }
                    ...
                    doAwaitResponse(reader_1.read(), function processChunk(response) {
                        if (!callbackCalled_1 && !response.rejected) {
                            ...
                            if (!result.done) {
                                ...
                                totalLength_1 += result.value[_DYN_LENGTH$4 ];
...
function AjaxHelperParseDependencyPath(logger, absoluteUrl, method, commandName) {
    ...
    if (absoluteUrl && absoluteUrl[_DYN_LENGTH$3 ] > 0) {
        var parsedUrl = urlParseUrl(absoluteUrl);
        target = parsedUrl.host;
        if (!name) {
            if (parsedUrl[_DYN_PATHNAME ] != null) {
                var pathName = (parsedUrl.pathname[_DYN_LENGTH$3 ] === 0) ? "/" : parsedUrl[_DYN_PATHNAME ];
...
var AnalyticsPlugin = /** @class */ (function (_super) {
    ...
    AnalyticsPlugin.Version = '3.3.11';
...
function DependencyEnvelopeCreator(logger, telemetryItem, customUndefinedValue) {
    ...
    var remoteDepData = new RemoteDependencyData(logger, bd.id, bd.target, bd[_DYN_NAME ], bd[STR_DURATION$1 ], bd.success, bd.responseCode, method, bd.type, bd.correlationContext, customProperties, customMeasurements);
...
//# sourceMappingURL=ai.3.3.11.gbl.js.map
